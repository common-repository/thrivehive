...
class JSON_API_Menus_Controller {	/**
	...
	**/
	public function add_menu_item(){
		...
		if(isset($_REQUEST['target_id'])){
			...
			$menu_item = $this->map_page($page, $title, $target);
		}
	...
	*@param string|null $title the title to use in the menu item, empty or null means use the post's title
	*@param string $target target attribute for the link (either '_self' or '_blank')
	*@api
	*@return array containing all the menu item objects
	**/
	private function map_page($page, $title, $target) {
		$menu_item = array(
	...
	**/
	public function set_logo() {
	  ...
	  //Image should be uploaded by this point
		...
		// Update both records for backwards (legacy) and forwards (Changeling) compatibility
	...
	**/
	...
	// This API is used to display the customer's logo under My Website -> Style in GMP
	...
	**/
	...
	public function set_theme_option() {
		...
		$optionstr = stripslashes($_REQUEST['option']);
	...
	public function set_all_theme_options() {
		...
		$optionsstr = stripslashes($_REQUEST['options']);
		$new_options = json_decode($optionsstr, true);
		if(empty($new_options)){
			$json_api->error($optionsstr);
		}
		$theme = basename(get_stylesheet_directory());
		$theme_options = get_theme_options_by_name($theme);
		if(isset($theme_options["theme"]))
		{
			...
			$res = add_theme_options($theme, serialize($new_options), $_REQUEST['version']);
		}
		return array('options' => $new_options);
	}
	...
	public function get_th_fontawesome_setting() {
		global $json_api;
		...
		$fontawesome = get_option("th_use_fontawesome");

		return array('option' => strtolower($fontawesome) === "1");
	}

	public function set_th_fontawesome_setting() {
		global $json_api;
		...
		if(!isset($_REQUEST['fontawesome'])){
			$json_api->error("You must provide an `fontawesome` to set for the option");
		}

		$value = strtolower($_REQUEST["fontawesome"]);

		update_option("th_use_fontawesome", $value === "true");

		return array('fontawesome' => $value);
	}

	public function get_website_phone() {
	    global $json_api;

        $nonce_id = $json_api->get_nonce_id('menus', 'get_website_phone');
        $nonce = wp_create_nonce($nonce_id);
        ...
        $phone_number = get_option("th_phone_number");
        return array('option' => $phone_number);
    }
	public function set_website_phone(){
	    global $json_api;

        $nonce_id = $json_api->get_nonce_id('menus', 'set_website_phone');
        $nonce = wp_create_nonce($nonce_id);
        ...
        if(!isset($_REQUEST['phone_number'])){
            $json_api->error("You must provide a `phone_number` to set for the option");
        }

        update_option("th_phone_number", $_REQUEST["phone_number"]);
        return array();
    }
