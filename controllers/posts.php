...
class JSON_API_Posts_Controller {
  ...
  **/
  public function create_post() {
   ...
   /* if (!$json_api->query->nonce) {
    ...
    $meta_values = strtolower(str_replace('\\', '', $_REQUEST['meta_values']));
    $meta_values = json_decode($meta_values);
    $_REQUEST['meta_values'] = $meta_values;
    ...
    $post_meta = get_post_meta($id); //So we can return the post meta on creation

    if(count($post_meta) > 0) //only add post meta if there is any. Empty associative array breaks downstream deserialization
    {
  ...
  **/
  public function update_post() {
    ...
    $id = $post->ID;
    $currentPostStatus = get_post_status($id);
    $categories = strtolower(str_replace('\\', '', $_REQUEST['categories']));
    $categories = json_decode($categories);
    $this->create_categories($categories);
    $_REQUEST['categories'] = $categories;
    ...
    if($slug_update == false && $currentPostStatus == 'publish' && $_REQUEST['status'] == 'publish')
    {
      $_REQUEST['datetime'] = get_the_time($date_format, $post->ID);
    }
  ...
  **/
  public function duplicate_post() {
    ...
    if (!$current_post_id) {
      $json_api->error("No post ID was sent.");
    }

    nocache_headers();
    $duplicated_post_id = duplicate_post_as_draft($current_post_id);
    if (!$duplicated_post_id) {
      $json_api->error("Duplication failed.");
    }
    $duplicated_post = get_post($duplicated_post_id);
    if (empty($duplicated_post_id)) {
      $json_api->error("Duplicated post not found.");
    }
    $post_to_return = new JSON_API_Post($duplicated_post);
    return array(
      'post' => $post_to_return
    );
  }
  ...
  public function upload_image() {
    ...
    if(!empty($_FILES['attachment'])){
      ...
      if($_FILES['attachment']['type'] == "application/pdf"){
        ...
        exec("convert -thumbnail x180 -background white -alpha remove -define pdf:use-trimbox=true \"{$pdf_url}[0]\" \"{$output_image}\" 2>&1", $output);
  ...
  **/
  public function get_post_meta(){
    ...
    $show_title = get_post_meta($_REQUEST['post_id'], 'th_show_title', true);
    ...
    return array('title' => $title, 'desc' => $desc, 'show_title' => $show_title);
  }
  ...
  **/
  public function get_post_preview(){
   ...
   /* if(!$json_api->query->nonce) {
    ...
    $res = site_url('/') . "?page_id=" . $_REQUEST['post_id'] . "&preview=true";
  ...
  **/
  ...
  public function get_page_genesis_layout(){
    ...
    if(!$layout){
      ...
      $default_layout = $genesis_options["site_layout"];
      return array("option" => $default_layout);
    }
  ...
  private function featured_image_response($image_id) {
    $image_url = wp_get_attachment_url($image_id);
    return array("featured_image" => array("url" => $image_url, "id" => $image_id));
  }
  ...
  public function set_featured_image(){
    ...
    if(!isset($_REQUEST["post_id"])){
      $json_api->error("You must specify the `post_id` of the post to access");
    }

    if(!isset($_REQUEST["image_id"])){
      $json_api->error("You must specify the `image_id` of the image to access");
    }

    $seo_options = get_option('aioseop_options');

    $seo_options["modules"] = array("aiosp_feature_manager_options" =>
                                    array("aiosp_feature_manager_enable_opengraph" => "on"),
                              "aiosp_opengraph_options" =>
                                    array("aiosp_opengraph_generate_descriptions" => "on",
                                          "aiosp_opengraph_defimg" => "featured",
                                          "aiosp_opengraph_types" => array("post", "page"),
                                          "aiosp_opengraph_post_fb_object_type" => "article",
                                          "aiosp_opengraph_categories" => "blog"));

    update_option('aioseop_options', $seo_options);

    update_post_meta($_REQUEST["post_id"], "_thumbnail_id", $_REQUEST["image_id"]);

    return $this->featured_image_response($_REQUEST["image_id"]);
  }
  ...
  public function get_featured_image(){
    ...
    $image_id = get_post_meta($_REQUEST["post_id"], "_thumbnail_id", true);

    if(!$image_id){
      return array("featured_image" => null);
    }

    return $this->featured_image_response($image_id);
  }
  ...
  public function delete_featured_image(){
    ...
    $image_id = delete_post_meta($_REQUEST["post_id"], "_thumbnail_id");

    return array();
  }

  public function get_global_header_image(){
    global $json_api;

    $header_image_url = has_header_image() ? get_header_image() : null;
    $header_image_enabled = get_theme_mod(CHANGELING_DISPLAY_HERO_SETTING, true);

    return array("global_header_image" => $header_image_url, "global_header_enabled" => $header_image_enabled);
  }

  public function update_post_show_title(){
    global $json_api;
    ...
    if(!isset($_REQUEST["show_title"])){
      $json_api->error("You must specify the `show_title` value of the post to update");
    }

    update_post_meta($_REQUEST["post_id"], "th_show_title", $_REQUEST["show_title"]);
  ...
  public function get_post_permalinks()
  {
    ...
    foreach ($posts as $post) {
      ...
      $res = array('url' => $permalink, 'path' => $path, 'name' => $post->post_title);
      array_push($links, $res);
    }
    return array('urls' => $links);
  }
