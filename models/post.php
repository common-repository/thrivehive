...
class JSON_API_Post {
  ...
  **/
  ...
  function save($values = null, $slug_update = false) {
    ...
    if (!empty($values['status'])) {
      ...
      if ($values['status'] == 'future') {
        // When editing a draft to be published in the future, this value must be true.
        $wp_values['edit_date'] = true;
      }
    }

    if (!empty($values['datetime'])) {
      $wp_values['post_date'] = $values['datetime'];
      $wp_values['post_date_gmt'] = $values['datetime'];
    }
    ...
    if($wp_values['post_type'] == 'page'){
      $wp_post = get_page($this->id);
    }
    else{
  ...
  function import_wp_object($wp_post) {
    ...
    $this->set_value('date', get_the_time($date_format, $this->id));
    ...
    if($wp_post->page_type){
      $this->set_value('page_type', $wp_post->page_type);
    }
